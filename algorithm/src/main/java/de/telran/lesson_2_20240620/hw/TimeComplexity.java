package de.telran.lesson_2_20240620.hw;
...
public class TimeComplexity {
    ...
    void test1(int n) {
        ...
        for (int i = 1; i <= n; i++) { // cost = 1 + 2 times = n
            for (int j = 1; j <= n; j++) { // cost = 3 times = 1
                System.out.println("*");
                break;
            }
        }
    } // best O(1), worst 1+2(n)*3+1 = O(n)
    ...
    void test2(int n) {
        ...
        int i;
        int j;
        for (i = 0; i < n; i++) { // cost = 1+2 times = n + 1
    ...
    void test3(int n) {
        int a = 0; int i; int j;
        for (i = n / 2; i <= n; i++) { // cost = 2+2 times = n/2
            for (j = 2; j <= n; j = j * 2) { // cost = 1+3 times = (n = 8) (j = 2,4,8) j^x = n = log(n)
                a = a + n / 2;
            }
        }
    } // 2+2*n/2*log(n) = O(n log n)

    // test 4
    void test4(int n) {
        int a = 0;
        int i = n;
        while (i > 0) { // cost = 1 times = i/2 = n^x (n = 32 = n/2 = 2^4 = 16); i = 32,16,8,4,2 = 2^5
            a += i;
            i /= 2;
        }
    } // 1+n/2^x = O(log n)
